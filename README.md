...
- `.well-known/atlasent-verifier-keys.json` — R2 (permit) + R3 (audit) Ed25519 verifier-keys JWKS. **Production keys only** — staging/non-production signing keys are deliberately never published here; see [`docs/STAGING_KEY_TRUST_POLICY.md`](docs/STAGING_KEY_TRUST_POLICY.md).
